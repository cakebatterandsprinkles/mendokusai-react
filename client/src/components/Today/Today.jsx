...
class Today extends Component {
    ...
    getToDos() {
        ...
        fetch(`/todo/today?date=${todayString}`, {credentials: "include"}).then((blob) => blob.json()).then((response) => {
            this
                .props
    ...
    deleteToDo(id) {
        fetch("/todo", {
            ...
            credentials: "include"
        }).then(() => this.props.updateToDos(this.props.todos.filter((todo) => todo._id !== id)));
    }
    ...
    updateToDo(todo) {
        ...
        const todoList = [
            ...this
                .props
                .todos
                .filter((oldToDo) => todo._id !== oldToDo._id),
            todo
        ];
        this
            .props
    ...
    handleSubmit() {
        ...
        this.addToDo({todo: this.state.addInput, status: "not done", date: todayString});
    ...
    componentDidMount() {
        ...
        this
            .getUserLocation()
            .then((data) => {
                this.setState({location: `${data.city}, ${data.region_name}, ${data.country_code}`});
            });
        this.getToDos();
    }
                ...
                <div className={classes.mainContainer}>
                    <div className={classes.flexContainerColumn}>
                        <div>
                            ...
                            <div className={classes.day}>{this.state.day === "Friday" || this.state.day === "Saturday" || this.state.day === "Sunday"
                                    ? "Yay! "
                                    : "Oh noes! "}
                                It's a {this.state.day}!</div>
                        </div>
                        <div className={classes.dataContainer}>
                            <div className={classes.flexContainerRow}>
                                <img src={LocationIcon} className={classes.locationIcon} alt="location icon"/>
                    ...
                    <div className={classes.flexContainerRowHeading}>
                      <div className={classes.heading}>{this.props.todos.length ? "Your day looks like this:" : "How to use this app:"}</div>
                      <div>
                          <img
                            src={AddIcon}
                            alt="add icon"
                    ...
                    <div className={classes.flexContainerColumnToDo}>
                        {this.props.todos.length ? this.renderToDoList(this.props.todos) : <div className={classes.warning}><div><span className={classes.star}>★</span> You can add new todos by clicking the plus icon on the right.</div>
                        <div><span className={classes.star}>★</span> Click on the checkbox icon to change the todo status to "in progress" or "done".</div><div><span className={classes.star}>★</span> To convert the temperature from Fahrenheit to Celcius, click on it.</div></div>}
                    </div>
                </div>
                <Modal
